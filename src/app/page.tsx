        ...
        <>
            ...
            <Cursors room={room} className="min-h-screen w-full" userCursorColor="tomato" renderCursor={renderCursor} zIndex={50}>
                ...
                <div className="min-h-screen flex flex-col relative">
                    <Header />
                    <main className="flex-1 flex flex-col items-center justify-center w-full relative px-4 py-6 pt-24 sm:p-4 sm:pt-28 md:p-6 md:pt-32">
                        <motion.div
                            initial={{ opacity: 0, scale: 0.95 }}
                            animate={{ opacity: 1, scale: 1 }}
                            transition={{ duration: 0.5 }}
                            className="relative w-full max-w-5xl mx-auto z-30 flex flex-col md:flex-row gap-8 md:gap-12 items-center justify-center"
                        >
                            <div className="flex flex-col items-center text-center space-y-6 sm:space-y-8 flex-1">
                                <div className="space-y-3">
                                    <h1 className="font-display text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-br from-foreground via-foreground/90 to-foreground/55 drop-shadow-2xl">
                                        CLICKER
                                    </h1>
                                    <p className="text-base sm:text-lg text-muted-foreground max-w-sm mx-auto px-2">
                                        Compete for the top spot in real time. Every click changes the board.
                                    </p>
                                </div>
                                <GiveClickButton />
                            </div>
